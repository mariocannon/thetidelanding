...
--
-- The questions follow the Life of Scoop "Data is power" survey (14 questions,
-- Naptown Scoop's), adapted for the Coast: postcodes instead of ZIP codes.
-- One column per question so answers can be counted with plain SQL
-- (group by household_income, etc.) instead of being unpacked from free text
-- the way poll_responses has to be.
--
-- Unlike poll_responses, rows are NOT anonymous: email is required so a
-- response can be matched back to the subscriber. That is the whole point of
-- the exercise — anonymous totals describe the audience, identified answers let
-- you sell a segment of it.
--
-- Only postcode, topics and email are required on the page; every other column
-- is nullable, because a half-finished survey is still worth keeping.
--
-- The value lists below are duplicated in src/pages/survey.astro. They are here
-- as well because the page inserts with the publishable key, so the database is
-- the only place that can actually refuse an answer the survey never offered.
create table public.survey_responses (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),

  -- Q1. Postcode. The page owns the list of covered postcodes, so it can gain a
  -- suburb without a migration; the shape is all that is enforced here.
  postcode text not null check (
    postcode ~ '^[0-9]{4}$' or postcode = 'None of the above'
  ),

  -- Q2. What they want to read — the only other required question.
  ...
  -- Q13. Only asked when Q12 is Yes — the page skips it otherwise, so a row
  -- with children_at_home = 'No' should have this null.
...
-- One response per reader: a second submission replaces nothing, it is simply
-- refused, which keeps the prize draw and the segment counts honest.
